(function() {
    ...
    */
    ...
    var cpf1 = cleanCPF('049-214 3421-1').split('');
    cpf1.splice(3, 0, '.');
    cpf1.splice(7, 0, '.');
    cpf1.splice(11, 0, '-');
    console.log(cpf1.join(''));
    
    console.log(
        cleanCPF('210.458.522-05').replace(/(\d\d\d)(\d\d\d)(\d\d\d)(\d\d)/, function(match, g1, g2, g3, g4) {
            return g1 + '.' + g2 + '.' +  g3 + '-' +  g4;
        })
    );
    
    console.log(
        cleanCPF('735 500 794 - 22').replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, function(match, g1, g2, g3, g4) {
            return g1 + '.' + g2 + '.' +  g3 + '-' +  g4;
        })
    );
    console.log(
        cleanCPF('101.123-131x32').replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, function(match, g1, g2, g3, g4) {
    ...
    */
    ...
    tags.replace(/<(\w+)>([\w\síáé]+)<\/\w+>/g, function(match, tag, text) {
